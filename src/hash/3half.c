/* 3half.c:  search for a good hash function based on exact multiplication by 3/2 */
...
/* Process Command Line Options */
	...
	if(--argc > 0){  /* Anything on command line ? */
			...
			/* -x option: for testing things */
				...
				m = 1;
				for(i=1;i<10000;i++)m = scramble(m);
				printf("%u\n",m);
...
/*   argc now gives a count of remaining arguments on command line,
	...
	if(read_file){ /* oops, I guess we did that */
		N = 0;
...
/* Sanity check: very useful for random test data */
	...
	for(i=0;i<N;i++){
		...
		v[i]=u[i]; /* save, because search algorithm below will change the v[i] */
	}
...
/*  Brute force search */
	...
	if(k < m){m = k; multiplier = t;}  /* New record */
...
/* Gather some statistics on the data */
	...
	if(k<K){
		...
		if(!quiet){
			...
			printf("Multiplier = %u, mask = %u\n",multiplier,mask);
		}
		for(i=0;i<N;i++){
/* Stuff that is done at "run time" */
			m = (u[i]*multiplier) & mask;
