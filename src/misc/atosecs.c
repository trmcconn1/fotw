...
int SecsToStr(double secs,  char *TimeString  )
{
     ...
     if (hh == 0) {
	  ...
	  if ( ss < 10.0 )
		sprintf(ptr,"%d:0%.1f",mm,ss);
	  else sprintf(ptr,"%d:%.1f",mm,ss);
          }
	else {
          if( mm == 0){
			if (ss < 10.0) 
				sprintf(ptr,"%d:00:0%.1f",hh,ss);
			 else sprintf(ptr,"%d:00:%.1f",hh,ss);
		       }
          else  
	  if ( mm < 10 )
		if ( ss < 10.0 )
			sprintf(ptr,"%d:0%d:0%.1f",hh,mm,ss);
		else    sprintf(ptr,"%d:0%d:%.1f",hh,mm,ss);
	else 
		if ( ss < 10.0) 
			sprintf(ptr,"%d:%d:0%.1f",hh,mm,ss);
		else
			sprintf(ptr,"%d:%d:%.1f",hh,mm,ss);
	 }
     return 0;
}
