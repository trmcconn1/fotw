...
int nics = NICS;
#define MAX_ORBITS 1024
int orbit_lib_size = 0; /* pointers to these stored in orbit_lib */
...
struct orbital_elements halley = {
		...
		2458880.5,      /* epoch (julian day) */
		27793.63935870046*3600*24, /* sidereal period (secs) */
...
/* A very parabolic, relatively recent named comet. We used this to
test the parabolic case. Note that if these elements are ported to
initial conditions and then back again the mean anomaly and 
mean motion fields will not match up at all. The reason is that
it will have been converted to an equivalent set of parabolic elements.
These elements are modelled as a hyperbola here and mean motion
and mean anomaly are treated quite differently in the parabolic case. */
...
struct orbital_elements seki_lines = {
		...
		1.0000003, /* eccentricity */
		11.481354*PI/180.0, /* argument of perifocus */
		.020813652204*PI/180.0, /* mean anomaly at epoch */
...
spherical angle coordinates are then constant 
...
We adopt the convention that if omega > 0 then motion is initially toward
...
struct orbital_elements freefall = {
		...
		SUN_CENTERED,
		REFER_TO_ECLIPTIC,
		0.0, /* longitude of the node (arbitrary) */
		-25.9466*PI/180.0, /* inclination = initial latitude here*/
		...
		(128.722)*PI/180.0,  /* +/- argument of perihelion =
					initial longitude here */
		2.57*AU_METERS, /* arg = radius at the epoch  here*/
		2458200.5,      /* epoch (julian day) */
...
struct initial_conditions luna2k_rv =
{
	"Orbit osculating J2K mean lunar orbit",
	EARTH_CENTERED,
	REFER_TO_ECLIPTIC,
	-358700808.395119,
	-160273206.362265,
...
struct orbital_elements *orbit_lib[MAX_ORBITS];
struct initial_conditions *ic_lib[MAX_ORBITS];
