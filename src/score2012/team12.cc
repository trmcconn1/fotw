...
Team::Team(Team &ATeam)
{
	...
	ReservePoints = ATeam.ReservePoints;
...
Team & Team::operator=(Team *ATeamPtr)
{
	...
	ReservePoints = ATeamPtr->ReservePoints;
...
void Team::PrintAll(fstream *out) const
{
	...
	*out << "Team Size: " << teamsize << endl;
	...
	int res_pts = 0;
	for (i = 0;i<teamsize; i++)
		if(Runners[i]->cleared < ReserveLimit){
			no_reserve++;
   ...
   if(ExcludedEventsHasMember(i)==0){
	...
	for(int j=0;j<teamsize;j++){
		if(Runners[j]->BestRuns[i] != NULL){
		    if((Runners[j]->cleared > 5)&&Runners[j]->compliant)
...
void Team::SetPoints()
{
	...
	for(int i =0;i<teamsize; i++){
		if((Runners[i]->cleared >= ReserveLimit)&&(Runners[i]->compliant))      // 6 different dates to count
		  TotalPoints += Runners[i]->TotalPoints;
		else ReservePoints += Runners[i]->TotalPoints;
	}
}

// BUG: Relays can, in theory, score more than 10 points. But this
// is unlikely we've elected not to fix it
...
ostream& operator<<(ostream &output,Team & ATeam)
{
	...
	for (int i = 0;i<ATeam.teamsize; i++)
		if(ATeam.Runners[i]->cleared < ReserveLimit ){
			no_reserve++;
			if(ATeam.Runners[i]->compliant)res_pts += ATeam.Runners[i]->TotalPoints;
		}
      output.fill(' ');
...
int ScoreFixedDMR(int AG1,int AG2,int AG3, int AG4, double time)
{
	...
	for(i=0;(i<NOEVENTS)&&(strcmp(EventNames[i],"400m")!=0);i++);
		indx2 = i;
	for(i=0;(i<NOEVENTS)&&(strcmp(EventNames[i],"mile")!=0);i++);
		indx3 = i;
	...
	*/
	...
	temp = mytrunc((20.0*((table[AG1][indx1]+table[AG2][indx2]+
		(table[AG3][indx1]+table[AG3][indx3]*1600/1609)/2+table[AG4][indx3]*1600/1609)/time)-10.0)/resolution);
