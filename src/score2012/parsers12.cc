...
#define ENDMONTH 5   /* Month of Last Day of Competition. Has always been May*/
#define ENDDAY 31


/* Return 1 if adate is within current competition year. Else return 0 */

int iswithinyear(struct tm adate)
{
	int y,m,d;

	y = adate.tm_year + 1900;
	m = adate.tm_mon + 1;
	d = adate.tm_mday;

	if(y == CompetitionYear){
		if(m < ENDMONTH) return 1;
		if(m == ENDMONTH)
			if(d <= ENDDAY)return 1;
		return 0;
	}
	if(y == CompetitionYear - 1){
		if(m > ENDMONTH) return 1;
		if(m == ENDMONTH)
			if(d > ENDDAY)return 1;
		return 0;
                ...
                switch(state) {
                        case ROOT:
                          ...
                          ParseDate(Tokens[1],&(rinfo->Date));
						   if(iswithinyear(rinfo->Date)==0)
							  fprintf(errstream,"Date on line %s is out of range\n",buffer);
        ...
        if(strcmp(Tokens[0],DATE) == 0) {
             ...
             /* Handle the crufty special event handled in Fall 1996 */
                                if(strcmp("XC",EventName)==0){
                                 ...
                                 if(strcmp("1600r",EventName)==0)rinfo->RelayLegEventNumber=4;
                                 if(strcmp("3200r",EventName)==0)rinfo->RelayLegEventNumber=7;
                                 if(strcmp("dmr",EventName)==0)rinfo->RelayLegEventNumber=index_dmr;
								 if(strcmp("4x1manley",EventName)==0)rinfo->RelayLegEventNumber=3;
								 if(strcmp("4x2manley",EventName)==0)rinfo->RelayLegEventNumber=5;
								 if(strcmp("4x3manley",EventName)==0)rinfo->RelayLegEventNumber=6;
                             ...
                             if(strcmp(Tokens[0],DISTANCE) == 0) {
                               if(count < 1){
                                 fprintf(errstream,"Syntax error: %s %s\n",
                                  ...
                                  strcpy(EventName,Tokens[1]);
                                 rinfo->RelayLegEventNumber = 0;
                                 if(strcmp("400r",EventName)==0)rinfo->RelayLegEventNumber=index_100m;
                                 if(strcmp("800r",EventName)==0)rinfo->RelayLegEventNumber=index_200m;
                                 if(strcmp("1600r",EventName)==0)rinfo->RelayLegEventNumber=index_400m;
                                 if(strcmp("3200r",EventName)==0)rinfo->RelayLegEventNumber=index_800m;
                                 if(strcmp("dmr",EventName)==0)rinfo->RelayLegEventNumber=index_dmr;
								 if(strcmp("4x1manley",EventName)==0)rinfo->RelayLegEventNumber=index_1xmanley;
								 if(strcmp("4x2manley",EventName)==0)rinfo->RelayLegEventNumber=index_2xmanley;
								 if(strcmp("4x3manley",EventName)==0)rinfo->RelayLegEventNumber=index_3xmanley;
                                 for(i=0;(i<NOEVENTS)&&(strcmp(EventNames[i],EventName)!=0)
                                        ;i++);
                                  if(i>=NOEVENTS){
                                        fprintf(errstream,"Unknown eventname: %s\n",EventName);
...
// See if team is found
         if(found == 0){ 
                fprintf(errstream,"No such team in roster file: %s Line %d\n",pinfo->teamname,*line);
                }
         else {
         if(*RunnerCount < MAX_ROSTER-1){
	char tempbuf[32] = "No Long Name Available"; 
...
ParseRcFile(FILE *rcfile, struct EXCLUDEDRELAYS *ExcludedRelays,
        ...
        char *TeamsFile, char *CsvFile, char *PrinterInitString, 
        ...
        int i;
          ...
          sscanf(LineBuffer,"%s %[^\n]",buffer,buffer2);
                ...
                if(strcmp(buffer,"TeamsFile")==0){
				 ...
				 if(strcmp(buffer,"CsvFile")==0){
                        strcpy(CsvFile,buffer2);
                        continue;
                        }
				  if(strcmp(buffer,"ReserveLimit")==0){
                        ReserveLimit = atoi(buffer2);
                        continue;
                        }
				...
				if(strcmp(buffer,"UseOldScoreTable")==0){
						legacy_table = 1;
						continue;
				}
				...
				if(strcmp(buffer,"EnforceOfficialMinimum")==0){
                        enforce_official_minimum=1;
                        continue;
                        }
				...
				if(strcmp(buffer,"TwoPointOldAgeMin")==0){
						two_pt_old_age_min = 1;
                        continue;
                        }
				...
				if(strcmp(buffer,"Use8kFor10k")==0){
						either_8k_or_10k = 1;
                        continue;
				}
if(strcmp(buffer,"ManleyRelaySubstitutes")==0){
						manleyor3200r = 1;
						manleyor1600r = 1;
						manleyor800r = 1;
                        continue;
