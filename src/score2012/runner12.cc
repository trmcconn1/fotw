...
const int EventFieldWidths[]={7,7,7,7,7,7,7,7,7,7,7,8,8,10,10,10,10,10,10};
const int RelayFieldWidths[]={7,7,8,8,8,7,8,8};
...
Runner::Runner()
{
	...
	best_10k_is_8k = 0;
	best_8k_is_10k = 0;
	best_3200r_is_manley = 0;
	best_1600r_is_manley = 0;
	best_800r_is_manley = 0;
	TotalPoints = 0;
	assert(Dob != 0);
	int i;
...
Runner::Runner(Runner& ARunner)
{
	...
	best_10k_is_8k = ARunner.best_10k_is_8k;
	best_8k_is_10k = ARunner.best_8k_is_10k;
	best_3200r_is_manley = ARunner.best_3200r_is_manley;
	best_1600r_is_manley = ARunner.best_1600r_is_manley;
	best_800r_is_manley = ARunner.best_800r_is_manley;
...
struct RunNode **Runner::GetRunsPointer(){
	return (RunNode **)Runs;
}
struct RelayNode **Runner::GetRelaysPointer(){
	return (RelayNode **)Relays;
}
void Runner::AddRelay( struct RelayNode *ARelay)
{
	...
	int k;
	Relays[i] = ARelay;
	int j = Relays[i]->LegDistance;
	    j = j-1; // because 55m is not a relay leg

		if(j+1 == index_dmr){ // handle as special case 
			j = NORELAYS  - 1;
		...
		(BestRelays[j]->time > Relays[i]->time))){
			...
			if(BestRelays[j])bestprev=BestRelays[j]->points;
			else bestprev = 0;
			
			// temp will hold the best existing score at this event, including
			// current one
		
			temp = Relays[i]->points;
			BestRelays[j]=Relays[i];
		}
		if(Relays[i]->points > bestprev)
			TotalPoints += temp-bestprev;
		}
		else {
	// NOT DMR
		/* See if this is a best relay at this distance */
	...
	/* How to adjust the score depends on whether this relay is linked to
	an "alternative" event. Currently, all 3 relays are linked to their manley
	full lap equivalents */

            int jj;    // alternate event index 

			if(j+1==index_800m)   // 4x800m
				jj = j - 1;  // BUG!: if any events are inserted this will all break
			
			if(j+1==index_3xmanley)
				jj = j + 1;
			if(j+1== index_400m)
				jj = j + 1;
			
			if(j+1== index_2xmanley)
				jj = j - 1;
				
			if(j+1== index_200m)
				jj = j + 1;
			
			if(j+1== index_1xmanley)
				jj = j - 1;		
		
		if(BestRelays[jj] && (BestRelays[jj]->points > bestprev))
			bestprev = BestRelays[jj]->points;
		if(BestRelays[jj] && (BestRelays[jj]->points > temp)){
			temp = BestRelays[jj]->points;
			
			
				if(jj+1== index_1xmanley)
					best_800r_is_manley=1;			
				if(jj+1== index_2xmanley)
					best_1600r_is_manley=1;	
				if(jj+1 == index_3xmanley)
					best_3200r_is_manley = 1;
				if(jj+1 == index_200m)
					best_800r_is_manley = 0;
				if(jj+1 == index_400m)
					best_1600r_is_manley = 0;
		...
		/* If this is not a best relay we only need to take into account its existence */

		switch(Relays[i]->type){
			case official:
				no_official++;
				if(cleared < ReserveLimit){   // check for number of dates required
				...
				break;
			case informal: // This should never occur per rules
			default: ;
		}
}
	   ...
	   double pts;
		...
		switch(Runs[i]->type){
			case official:
				no_official++;
				if(cleared < ReserveLimit){   // check for number of dates required
				cleared++;  // but take away if she already has credit:
				temp = cleared;
				for(k=0;k<i;k++) 
		...
		if(j == index_5M){
			j = index_8k;  // pretend 5M is an 8k (will be scored as 5M)
			fprintf(errstream,"5 Mile run for %s recorded as 8k\n",ShortName);
		}
		...
		temp = mytrunc( pts );
		if(temp > 10/resolution) temp = (int) 10/resolution;
		if(age < 85)
			Runs[i]->points = ( (double)temp < 1/resolution ? (int)(1/resolution) : temp );
		else{
			if(two_pt_old_age_min)
				Runs[i]->points = ( (double)temp < 2/resolution ? (int)(2/resolution) : temp );
			else 
				Runs[i]->points = ( (double)temp < 1/resolution ? (int)(1/resolution) : temp );
		}
		Runs[i]->pts = pts;
		...
		/* See if this is a best best run at this distance */
		       ...
		       if(((either_100m_or_55m || exclusive_extremes)&&((j==index_15k)||(j==index_55m)||(j==index_100m)))||( either_mile_or_metric &&((j==index_1500m)||(j==index_mile)))||(either_8k_or_10k &&((j==index_8k)||(j==index_10k)))||((either_10m_or_15k || enable_combined_dr)&&((j==index_hmar)||(j==index_mar)||(j==index_15k)||(j==index_10M)))){	                         
...
/* Code to handle score better of 8k and 10k points. Rule
	     *  change in 2009-10 */

                          if(either_8k_or_10k && ((j==index_8k)||(j==index_10k))){
                       		if(BestRuns[index_10k]==0){
			 		if(BestRuns[index_8k]==0){
			  			temp = 0;
						if(j==index_10k)
							best_8k_is_10k = 1;
					}
			 		else {
						temp = BestRuns[index_8k]->points;
						if((j==index_10k)&&(Runs[i]->pts 
							> GetPoints(index_8k)))
							best_8k_is_10k = 1;
						else
							best_8k_is_10k = 0;
			        	...
			        	if(BestRuns[index_8k]==0){
			 			temp = BestRuns[index_10k]->points;
						if((j==index_8k)&&(Runs[i]->pts
							> GetPoints(index_10k)))
							best_8k_is_10k = 0;
						else best_8k_is_10k = 1;
					}
					else {
						temp = ( BestRuns[index_10k]->points >
			   				BestRuns[index_8k]->points ?
			    				BestRuns[index_10k]->points :
			    				BestRuns[index_8k]->points ) ;
						if((Runs[i]->pts > GetPoints(index_10k))&&
							(Runs[i]->pts > GetPoints(index_8k)))
							if(j == index_8k)
							   best_8k_is_10k = 0;
						        else
							   best_8k_is_10k = 1;
	   ...
	   /* temp now has the better of the 8k and 10k points */
                       		if(Runs[i]->points > temp)
			 		TotalPoints += Runs[i]->points - temp;
					  ...
					  ||(j==index_mar)))
				  {
					  ...
					  e = Runs[i]->points;
						...
						if((e>a)&&(e>b)&&(e>c)&&(e>d)){// new best distance run
							...
							// Find the max of a,b,c,d so we can credit point total
							// with e-m
							temp = (a > b)? a : b;
							if(c > temp) temp = c;
							if(d > temp) temp = d;
...
char *Runner::GetSex() 
{
	return Sex;  // if only it were this easy!
}
...
void Runner::PrintAll(fstream *out) const
{
	...
	tm_now.tm_mday = time_now.wDay;
	tm_now.tm_mon = time_now.wMonth - 1;
	tm_now.tm_year = time_now.wYear - 1900;
#endif
...
#ifdef UNDER_CE
		<< " " << AgeGroups[AgeGroupNo(Dob,&tm_now,Sex)] <<
#else
	...
	*out << (no_runs + no_relays) <<  " performances to date" <<
	...
	if(cleared >= ReserveLimit) *out << "This runner has run on at least req'd official dates." << endl;
...
{   char buffer[BUFFERSIZ];
	int i,j,compliant=1,count=0;

	/* Check to make sure this runner is compliant with the rule that only
	allows up to 4 outside times to count in the results */
 ...
 // Left Justify name:
	...
	if(ARunner.cleared < ReserveLimit) output << "[";
			else output << " ";
	}
	...
	for(i = 0;i<NOEVENTS-NORELAYS;i++){       // Display Results
	 if(ExcludedEventsHasMember(i) == 0){
		...
		/* Logic below is hard to read. We want to enter -------
				  ...
				  Also: in 2009 we added the rule that 8k and 10k can count for each other */
		...
		if(either_8k_or_10k){
			if((i==index_8k)&&ARunner.best_8k_is_10k)j=index_10k;
			if((i==index_10k)&&!ARunner.best_10k_is_8k)j=index_8k;
		}
		...
		if(ARunner.BestRuns[j]==0){
			if((either_10m_or_15k&&((i==index_10M)||(i==index_15k)))||(either_8k_or_10k&&((i==index_8k)||(i==index_10k)))||(either_100m_or_55m&&((i==index_100m)||(i==index_55m)))||(either_mile_or_metric&&((i==index_1500m)||(i==index_mile))))
		  ...
		  if(either_10m_or_15k || either_mile_or_metric || either_100m_or_55m){
			...
			if((i==index_8k)||(i==index_10k)){
				if(((i==index_10k)&&ARunner.best_10k_is_8k)||
				((i==index_8k)&&ARunner.best_8k_is_10k)){
					output.width(1);
					output << "*";
				}
				else {
	...
	for( i = 0;(i<NORELAYS); // +1 offsets us from the 55m which is not
		...
		// j records the index where we get the result from: same deal as 
		//  individual events - to account for linked events
		j = i;
		if((i+1==index_800m)&&ARunner.best_3200r_is_manley)j=i-1;
		if((i+1==index_400m)&&ARunner.best_1600r_is_manley)j=i+1;
		if((i+1==index_200m)&&ARunner.best_800r_is_manley)j=i+1;

	  /* if((i+1==index_200m)||(i+1==index_400m)||(i+1==index_800m)){ */
		if(!ExcludedRelaysHasMember(i)){ 

		if((ARunner.BestRelays[i]==0)&&(ARunner.BestRelays[j]==0)){
		...
		::SecsToStr(ARunner.BestRelays[j]->time,buffer);
		output << buffer;
		output.width(1);
		output << "=";
		output.width(3);
		if(i==j)
		  sprintf(buffer,"%3.1f", ((double)(ARunner.BestRelays[i]->points))*resolution);
			...
			sprintf(buffer,"%3.1f", ((double)(ARunner.BestRelays[j]->points))*resolution);
		output << buffer;
		if(i!=j){
			output.width(1);
			output << "*";  // if actual event different than nominal event
		}
		}
	      }
	      output.fill(' ');
...
int AgeGroupNo(struct tm* DOB, const struct tm* rundate, const char *Sex)
{
	...
	if((DOB->tm_mon < rundate->tm_mon)||((DOB->tm_mon==rundate->tm_mon)&&(DOB->tm_mday
	   ...
	   return 9;  // open male if all else fails 
}
...
double score(int AgeGroup, int EventNo, double time)
{
	...
	s = (20.0*(table[AgeGroup][EventNo]/time) - 10.0)/resolution;
	if(s > 10.0/resolution) s = 10.0/resolution;
	return s;
