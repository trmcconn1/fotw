/* Kepler.c:
   ...
   For hyperbolic orbits the analogue of kepler's equation is

	M = -E + e sinh E

   If e > 1 then we only at present support the Newton's method option 
...
Here we solve different "kepler's equations depending on whether e < 1
(E - EsinE = M), e = 1 (E^3/3 + E = M), or e > e (-E + esinh(E) = M).
...
double newton(double E, double e, double M, int reset)
{
	...
	if(e > 1.0) return E - (M - e*sinh(E) + E)/(1.0 - e*cosh(E));
	if(e == 1.0) return E - (E*E*E/3.0 + E - M)/(1.0 + E*E); 
}
...
main(int argc, char **argv)
{
	...
	e = atof(argv[i]);
	if(e > 1.0)
	method = (double(*)(double,double,double,int))methods[INEWTON];
	else
