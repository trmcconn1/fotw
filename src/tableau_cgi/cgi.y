...
extern int tableau_cgi(char *, char *, int);
extern int cgilex();
extern void cgierror(char *);
int HexToA(char *);
extern FILE *cgiin;
pid_t pid;
...
/* Sort the Pairs array alphabetically on the Name field */
	...
	if(Value("TYPE")==NULL){
		printf("Client did not specify which type of display\n");
		return 1;
	}
	if(Value("WFF")==NULL){
		printf("Client did not specify a wff\n");
		return 1;
	}
/*
	if(Value("SIGN")==NULL){
		printf("Client did not specify a sign\n");
		return 1;
	}
*/
	stderr = stdout; /* The routines print their error
                            messages to stderr, but some web servers ignore
			    this */
 
	if(strcmp(Value("TYPE"),"TRUTHTABLE")==0)
		tableau_cgi(Value("WFF"),"-t",0);
	else if(strcmp(Value("TYPE"),"TABLEAU")==0){
		if(strcmp(Value("SIGN"),"TRUE")==0)
			tableau_cgi(Value("WFF"),"-a",1);
		else
			tableau_cgi(Value("WFF"),"-a",0);
	}
	else printf("Unknown client value, %s, for TYPE type", Value("TYPE"));
