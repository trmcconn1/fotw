...
const int EventFieldWidths[]={7,7,7,7,7,7,7,7,8,8,10,10,10,10,10};
...
{          int i = no_runs++;
	   ...
	   double pts;
		...
		/* See if this is a best best run at this distance */
			...
			/* Code to handle various special cases */
		       ...
		       if((j==index_55m)||(j==index_1500m)||(j==index_mile)||(j==index_15k)||(j==index_10M)||(j==index_100m)){	                         
	   ...
	   /* temp now has the better of the 100m and 55m mile points */
            ...
            /* Code to handle scoring better of 15K and 10M points */
...
void Runner::PrintAll(fstream *out) const
{
	...
	cout << ShortName << " ( " << LongName << " )" << buffer
		<< " " << AgeGroups[AgeGroupNo(Dob,localtime(&now),Sex)] <<
		" " << TeamName << endl << endl;
	cout << (no_runs + no_relays) <<  " performances to date" <<
	"<" << no_official << " official, " << (((no_runs + no_relays-
	no_official) - no_informal)) << " outside, " <<
	no_informal << " informal>: " << endl;
        if(special) cout << "This runner ran the xc preseason event. Bully for 'em." << endl;
	if(cleared >= 4) cout << "This runner has run on at least 4 official dates." << endl;
        else cout << "This runner has run on " << cleared << " official dates." << endl;
	for(  i = 0;i<no_runs;i++){
	  sprintf(buffer," %d/%d/%d ", Runs[i]->date.tm_mon+1,
		 Runs[i]->date.tm_mday,Runs[i]->date.tm_year);
	  ::SecsToStr(Runs[i]->time,buffer2);
	  if(BestRuns[Runs[i]->distance]==Runs[i])cout << "*";
	  else cout << " ";
	cout << buffer << EventNames[Runs[i]->distance] <<
	...
	for( i = 0; i<no_relays;i++){
	  ...
	  if(BestRelays[Relays[i]->LegDistance - 1]==Relays[i])cout << "*";
	  else cout << " ";
	cout << buffer << EventNames[Relays[i]->distance] <<
     ...
     "=" << ((double)(Relays[i]->points))*resolution 
	  ...
	  cout << "Total Points: " << ((double)(TotalPoints))*resolution << endl;
