...
/* Conversions involving GPS coordinates. There are two common formats
for latitudes and longitudes in geodesy. One gives degrees, minutes, seconds
and fractions of a second (normally to 1 decimal point accuracy). The
other gives degrees, minutes, and fractions of a minute, normally to 
3 decimal place accuracy. Optionally, a letter from among
N,S,E,W,n,s,e,w may either precede or follow the dms expression. It is
convenient to convert such expressions into the normalized form expected
by the gpx schema, which is just degrees to any arbitrary number of decimal
places in appropriate positive/negative ranges */

/* Convert deg min sec format to decimal degrees through the pointer
argument. Return Integer code for NLAT, SLAT, WLONG, ELONG, INDEFINITE, or NOTWFF (not
well-formed */

/* values of parsing flag */
...
int dms2s(char *s,double *d)
{
	char *p;
	char c;
	int i,n;
	char buf[ARG_SIZE];
	int flag = PREFIX; /* this tracks where we are in parsing */ 
	int ws = 0;  /* track multiple whitespace */
	int dp = 0; /* track decimal point */
	int rval = INDEFINITE;
	...
	if(!n)return NOTWFF;
	if(n >= ARG_SIZE){
		fprintf(stderr,"dms2s: argument too long\n");
		return NOTWFF;
	}

/* scan from the beginning to end of s dropping leading and trailing 
whitespace, nondigits/decimal points,  multiple internal whitespace. Make a note in the return value
according as a letter is encountered. Convert single internal whitespace
char to : for use by secs.c */
	...
	for(i=0;i<n;i++){
		...
		switch(c){
			...
			if((flag == PREFIX)||(flag == SUFFIX))
				break; /* skip all ws here */
			if(ws) break;  /* skip multiple whitesp in gaps */
			if(flag == DEGFIELD){
				flag = FIRSTGAP;
				ws = 1;
				*p++ = ':';
...
/* We are being very generous. These can occur anywhere, as many times as
   desired */
			case 'N':
			case 'n':
				rval = NLAT;	
		...
		if(rval == NOTWFF)break;
	} /* End For */

	*p++ = '\0';
	secs(buf,d);

	if(debug)fprintf(stderr,"dms2s:in=%s,out=%s,rval=%d\n",s,buf,rval);

	return rval;
}

/* Similar to dms2d above except the input is in ddmm.m format with an
optional ' following the mm.m */
 
int dm2s(char *s,double *d)
{
	char *p;
	char c;
	int i,n;
	...
	if(!n)return NOTWFF;
	if(n >= ARG_SIZE){
		fprintf(stderr,"dm2s: argument too long\n");
		return NOTWFF;
	}
...
/* We are being very generous. These can occur anywhere, as many times as
	...
	*p++ = '\0';
	if(debug)fprintf(stderr,"dm2s:in=%s,out=%s,rval=%d\n",s,buf,rval);
	secs(buf,d);
	*d = 60.0*(*d); /* secs interprets buf as mins/secs intstead of
                            hrs:mins */
	if(debug)fprintf(stderr,"dm2s: seconds = %g\n",*d);
