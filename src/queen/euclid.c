...
integer my_gcd(integer a, integer b, integer *c, integer *d)
{
	integer q,r,t,C,D,rval;
	integer sgn_a = 1,sgn_b = 1, swap = 0;
...
main(integer argc, char **argv)
{
	...
	g = my_gcd(a,b,&c,&d);
	if(g == -1){
		fprintf(stderr,"euclid: gcd(%ld,%ld) is not defined.\n",a,b);
		return 1;
	}
