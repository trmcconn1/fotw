...
integer
is_happy(integer n, integer rdx)
{
	integer j,old_n,loops=10;	
...
main(integer argc, char **argv)
{
	...
	if(lflag){
		...
		if(eflag) printf("Ecstatic numbers (radix <= %ld) <= %ld:\n",
						radix,num);
		else printf("Happy numbers (radix = %ld) <= %ld:\n",
						radix,num);
