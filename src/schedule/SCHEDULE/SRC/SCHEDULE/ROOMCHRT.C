...
/* Options -mwf and -tth build roomcharts for multiple days on one page. */
/* If -mwf, it is assumed the dates entered are a monday, and if -tth it
   is assumed the dates entered are a tuesday */
...
int makeboxes(FILE *, struct RoomStruct *, time_t, time_t, int,int);
...
int roomchart(SHELLSTREAMS)
{
	...
	int BOX_HEIGHT = DEFAULT_BOX_HEIGHT
	int meets = 0;
	int i,j,k,l;
	int iterations = 1;  /* used with mwf, tth flags */

/* handle options */

	if(numtokens >= 2)
		if(*(Tokens[1]->text) == '-'){
			if(strcmp(Tokens[1]->text,"-tth")==0)
				iterations = 2;
			if(strcmp(Tokens[1]->text,"-mwf")==0)
				iterations = 3;
		}
	
	BOX_HEIGHT = iterations;
	if(numtokens != 1 && numtokens != 5 && numtokens != 6) return 1;
...
/* Get info from the user */
	...
	if(numtokens >= 5 ){
	start = ParseDate(Tokens[numtokens-4]->text,Tokens[numtokens-3]->text);
	end = ParseDate(Tokens[numtokens-2]->text,Tokens[numtokens-1]->text);
	}
	...
	for(i=0;i<numrooms;i++){
		if(Rooms[i]->numclasses > 0){
	          fprintf(shell_ostream,"label %d \"%s\"\n",j,
                      Rooms[i]->name);
		makeboxes(shell_ostream, Rooms[i],start, end, j,l);
		j++;
		}
	}
      start += 2*SECS_PER_DAY; /* Mon --> Wed, Tue --> Thur, etc */
      end   += 2*SECS_PER_DAY;
      }
	return 0;
}
...
of commands of the form box(a,b,c,d,string,n), one for each time slot that
...
int makeboxes(FILE *output, struct RoomStruct *rmptr, time_t start,
                   time_t end, int pos , int txt_vert_offset)
{
	int i,j;
	time_t k,l;
	...
	for(i=0;i<rmptr->numclasses;i++)	
		...
		for(j=0;j<(rmptr->classes[i])->times->number;j++){
			...
			if(k<=end && l>=start)
				fprintf(output,"box %d %d %lu %lu \"%s %s\" %d\n",
					pos, pos + BOX_HEIGHT,
					(k-start)*HOR_PER_SEC,
