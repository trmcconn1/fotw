...
/* sort courses in source order for display */
	...
	for(i=0;i<schedptr->numassignments;i++){
		...
		if(strcmp(schedptr->assignments[i]->comment,"")!=0)
		  fprintf(shell_ostream,"%s\n",schedptr->assignments[i]->comment);
		}
	return 0;
	}
...
int course(SHELLSTREAMS)
{
	...
	if(numtokens == 2 && strcmp(Tokens[1]->text,"end")==0){
                if(strcmp(GetShell("CLASS-LEVEL"),"0")!=0)
                  fatal(stderr,"Unmatched subclass. \n");
...
int assign(SHELLSTREAMS)
{
	...
	if(numtokens == 1){  /* list all departmental assignments */
		...
		if(strcmp(CurrentSchedule->assignments[i]->comment,"")!=0)
		  fprintf(shell_ostream,"%s\n",CurrentSchedule->assignments[i]->comment);
	        }
		return 0;
	}
...
/* see if there is an existing course by this name */
		...
		if(strcmp(CrsPtr->assignments[i]->comment,"")!=0)
		  fprintf(shell_ostream,"%s\n",CrsPtr->assignments[i]->comment);
		}
                return 0;
	}
...
/* else attach this assignment to department */
	...
	set("ASSIGNMENT",AssgnPtr->name,3,shell_ostream);
	CurrentAssignment = AssgnPtr;
	return 0;
...
int comment(SHELLSTREAMS)
{
	...
	if(GetShell("CLASS")==NULL || CurrentClass == NULL){
 		...
 		if(GetShell("ASSIGNMENT")==NULL || CurrentAssignment == NULL){
		 myerror(shell_errorstream,"comment: Nothing to attach to.\n");
	         return 0;
		}
		else {
		 SAFECPY(CurrentAssignment->comment,Tokens[1]->text,LONGNAME);
		 return 0;
                }
	    }
	    else { 
...
PrintCourse(struct CourseStruct *CrsPtr,FILE *shell_ostream)
{
	...
	/* Print all classes of this course */
         ...
         /* Classes will be listed alphabetically by name */

                krqsort((void **)CrsPtr->classes,0,CrsPtr->numclasses-1,(int(*) (void *,void *))
                CompareClassesByName);
...
/* Print time, if known */
	...
	if(aclass->times == NULL)
		sprintf(buffer, "%-20s ",MissingData);
...
/* Print bldg and room if known */
	if(aclass->room == NULL)
		sprintf(buffer, "%s",MissingData);
			...
			aclass->room->number);
	fprintf(shell_ostream,"%-10s",buffer);

/* Print teacher, if known */

	if(aclass->teacher == NULL)
		sprintf(buffer, "%s",MissingData);
	else  sprintf(buffer,"%s",aclass->teacher->shortname);
	fprintf(shell_ostream,"%-20s",buffer);


/* We want stuff below to come out lined up on the right  */
/* Undo the effect of indentation */
	for(i=0;i<3-level;i++)fprintf(shell_ostream,"%s",indent);

	if(aclass->available == 0)fprintf(shell_ostream,"%4s","N");
...
/* Print assignments attached to this class, if any */
	...
	for(i=0;i<aclass->numassignments;i++){
		...
		if(strcmp(aclass->assignments[i]->comment,"")!=0)
		  fprintf(shell_ostream,"%s\n",aclass->assignments[i]->comment);
	}
