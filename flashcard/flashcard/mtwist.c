...
#endif
#ifdef UNDER_CE
#include <windows.h>  /* For declaration of GetTickCount() */
#endif
...
static uint32_t mts_devseed(
    ...
    if (ranfile != NULL)
	{
	setvbuf(ranfile, NULL,0x0004,0);/*_IONBF=0x0004 */
	for (nextbyte = 0;
...
#else
	/* TMC: CE doesn't support the usual windows system time calls */
	randomunion.randomvalue = GetTickCount();
#endif
#else /* WIN32 */
